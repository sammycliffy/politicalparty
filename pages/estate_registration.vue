<template>
  <div class="login-page">
    <div class="form" id="estateForm">
      <form class="login-form" method="post" @submit.prevent="addEstate">
...
export default {
  ...
  methods: {
    ...
    async addEstate({ $axios }) {
      this.loading = true;
      const data = {
        estate_name: this.estateName,
        estate_address: this.estateAddress,
      ...
      this.$axios
        ...
        .then(response => {
          if (response.status === 200) {
            ...
            this.$store.dispatch("facility", JSON.parse(facility));
          }
        })
        .catch(error => {
          this.loading = false;
